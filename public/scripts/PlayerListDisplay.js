...
class PlayerListDisplay {
  constructor({
    ...
    if(!(game instanceof AstrologyBingoGameController)) {
      throw new Error(`PlayerList needs a an AstrologyBingoGameController; instead received: ${game} (type: ${typeof game}, class: ${game?.__proto__?.constructor})`)
    }
    this.game = game;

    if(!isElement(mountNode)) {
      throw new Error(`PlayerList needs an element to mount into; instead received: ${mountNode} (type: ${typeof mountNode}, class: ${mountNode?.__proto__?.constructor})`)
    }
    this.mountNode = mountNode;
  }

  render(mount=this.mountNode, renderFn) {
